...
const isLoggedin = async (req, res) => {
  try {
    ...
    const user = await userModel.findById(decoded._id); // Use the decoded userId
...
// const isLoggedin = async (req, res) => {
//   try {
//     const userId = req.body._id; // Get the _id from the request body directly
...
//   try {
//     const { _id, name, email } = req.body; // Expecting the user ID along with the name and email to be in the request body

//     // Validate input
//     if (!_id || !name || !email) {
//       return res.status(400).send({ message: "User ID, name, and email are required", success: false });
//     }
...
//     // user.role = role || user.role; // if you need role to be updated
